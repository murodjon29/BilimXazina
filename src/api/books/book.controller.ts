import { BadRequestException, Body, Controller, Get, HttpStatus, Post, Query, UploadedFiles, UseInterceptors } from '@nestjs/common';
...
export class BookController {
 ...
 @ApiOperation({ summary: 'create book', description: 'Creates a new book with details and images' })
  ...
  @UseInterceptors(FilesInterceptor('images'))  
    async createBook(@Body() createBookDto: CreateBookDto, @UploadedFiles() files?: Express.Multer.File[] | any) {
        if (!files || files.length == 0) {
            throw new BadRequestException('At least one image is required');
        }
        return this.bookService.createBook(createBookDto, files);
    }
